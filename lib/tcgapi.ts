...
export async function fetchFormCardLastResort(displayName: string): Promise<string | null> {
  const cards = await fetchAllPages(`name:"${displayName}" -rarity:Common -rarity:Uncommon`);
...
export async function fetchTcgLastResort(
  ...
  const results = await Promise.all(
    pokemon.map(async ({ id, name }, i) => {
      ...
      const cards = await fetchAllPages(`name:"${displayName}" -rarity:Common -rarity:Uncommon`, true);
      const candidates = cards.filter(c =>
