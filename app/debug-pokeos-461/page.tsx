...
// Known Gen 1 dex numbers for reference
...
export default function DebugPokeos461Page() {
  ...
  const cards = Array.from({ length: 103 }, (_, i) => i + 1);

  return (
    <div className="p-6 bg-gray-900 min-h-screen">
      <h1 className="text-white text-2xl font-bold mb-2">PokéOS Set 461 — identify Gen 1 cards</h1>
      <p className="text-gray-400 mb-2 text-sm">Card #80 = Magnemite (dex 81). Which card numbers are Gen 1 Pokémon?</p>
      <p className="text-yellow-400 mb-6 text-xs">If card # matches dex # it&apos;s highlighted green. Hover for dex guess.</p>
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-3">
        {cards.map((n) => {
          const dexGuess = GEN1_DEX[n];
          return (
            <div
              key={n}
              className={`flex flex-col items-center gap-1 rounded p-1 ${dexGuess ? "bg-green-900/40 ring-1 ring-green-500" : ""}`}
              id={`c${n}`}
              title={dexGuess ? `Dex guess: ${n} = ${dexGuess}` : ""}
            >
              ...
              {dexGuess && <span className="text-green-300 text-[9px] text-center leading-tight">{dexGuess}</span>}
            </div>
          );
        })}
      </div>
